...
% Output the original IV in file IV_ORI.dat
fileid=fopen(strcat('m',num2str(n),'_IV_ORI.dat'),'wt');
for i=1:www,
...
grid
axis tight
ht=title(strcat(p,'\\m',num2str(n),' Original IV, V=',num2str(vf),' V, I=',num2str(fi),' nA'));
set(ht,'fontsize',12);
%legend(le)

% Output the Averaged spectra in file IV_AVE.dat
fileid=fopen(strcat('m',num2str(n),'_IV_AVE.dat'),'wt');
for i=1:www1,
    fprintf(fileid, '%f\t', xz(i)); 
    for j=1:ng,
        if gs>1,
