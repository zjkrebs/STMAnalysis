% FT_FlatdIdVMap_yuanbo(n)
% Fourier Transform the dIdVMap for file mn_ori (flattened by WSxM)

function FT_FlatdIdVMap_yuanbo(fname)

zs3 = load(fname);
[nx_zs3, ny_zs3] = size(zs3);
n_zs3 = int32(sqrt(nx_zs3));

% reconstruct 2D z matrix
zs2 = reshape(zs3(:,3),n_zs3,n_zs3);
zs2 = zs2 - mean2(zs2);
zs2 = zs2';

% reconstruct the x, y vectors
x = zs3(1:n_zs3,1);
y = x;
...
dx = max(x)/nx2;  % sampling length x
dy = max(y)/ny2;  % sampling length y
