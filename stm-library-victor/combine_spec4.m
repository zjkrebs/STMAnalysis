% function combine_spec4(s1,s2,v1,v2)
% plot the differences as well
function combine_spec4(s1,s2,v1,v2,v3,v4)
p=strrep(cd,'\','\\');
...
xxx4=length([v1 v2 v3 v4]);
        plot(xz,mean(m(1:xxx1,:)),'b',xz,mean(m((xxx1+1):xxx2,:)),'b:','linewidth',1.5)
        plot(xz,mean(m((xxx2+1):xxx3,:)),'r',xz,mean(m((xxx3+1):xxx4,:)),'r:','linewidth',1.5)
        plot(xz,mean(m(1:xxx1,:))-mean(m((xxx2+1):xxx3,:)),'g',xz,mean(m((xxx1+1):xxx2,:))-mean(m((xxx3+1):xxx4,:)),'g:','linewidth',1.5)
        hold off
set(gca,'fontsize',18)
grid
axis tight
