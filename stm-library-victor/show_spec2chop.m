...
% 2nd harmonic data
% b = # of spectra per line


function show_spec_i(n,gs,cn,Vmax,step)
p=strrep(cd,'\','\\');
...
for i=1:ng
    if gs>1,
        ...
        %for j=1:cn
            chopy=mean(cs0((i-1)*gs+1:(i-1)*gs+gs,:));
            chop=reshape(chopy,n2/cn,[]);
           
            plot((-1).*(xz(1:n2/cn)+n2/cn).*Vmax./(n2/cn),mean(chop,2),cc(mod(i,7)+1),'linewidth',1.5)
            %plot(xz,mean(cs0((i-1)*gs+1:(i-1)*gs+gs,:)),cc(mod(i,7)+1),'linewidth',1.5)
        %end
    else
...
for i=1:ng,
    if gs>1,
        %plot(xz,mean(cs1((i-1)*gs+1:(i-1)*gs+gs,:)),cc(mod(i,7)+1),'linewidth',1.5)
            chopy=mean(cs1((i-1)*gs+1:(i-1)*gs+gs,:));
            chop=reshape(chopy,n2/cn,[]);
