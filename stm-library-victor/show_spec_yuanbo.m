...
% Output the original spectra in file SPEC_ORI.dat
fileid=fopen(strcat('m',num2str(n),'_SPEC_ORI.dat'),'wt');
for i=1:www,
...
% Output the Normalized and Averaged spectra in file SPEC_AVE_NOR.dat
fileid=fopen(strcat('m',num2str(n),'_SPEC_AVE_NOR.dat'),'wt');
for i=1:n2,
    fprintf(fileid, '%f\t', xz(i)); 
    for j=1:ng,
        if gs>1,
...
% Output the Averaged spectra in file SPEC_AVE.dat
fileid=fopen(strcat('m',num2str(n),'_SPEC_AVE.dat'),'wt');
for i=1:www1,
    fprintf(fileid, '%f\t', xz(i)); 
    for j=1:ng,
        if gs>1,
