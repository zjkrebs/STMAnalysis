...
% and it shows abs() of the average of cs0 (either i(v) or di/dv(v) if taking 
% 2nd harmonic data
...
%LOOK OUT FOR THIS SMOOTH FUNCTION!!!  WHO PUT IT HERE??????????????
%for i=1:qqq,
%    cs0(i,:)=smooth(cs0(i,:));
%end
...
for i=1:ng
    if gs>1,
        plot(xz,abs(mean(cs0((i-1)*gs+1:(i-1)*gs+gs,:))),cc(mod(i,7)+1),'linewidth',1.5)
    else
        plot(xz,abs(cs0(i,:)),cc(mod(i,7)+1),'linewidth',1.5)
    end
%        le(i,1)=num2str(i);
end
hold off  
